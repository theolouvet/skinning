...
skeleton_geometry local_to_global(skeleton_geometry const& sk_local,skeleton_parent_id const& parent_id)
{
    ...
    sk_global.push_back(sk_local[0]);
    skeleton_joint joint_local;
    skeleton_joint joint_parent_global;
    vec3 pparent;
    vec3 p;
    quaternion qparent;
    quaternion q;
    for ( int i=1; i < N; i++)
    {
        ...
        joint_local = sk_local[i];
        joint_parent_global = sk_global[parentID];
        pparent = joint_parent_global.position;
        p = joint_local.position;
        qparent = joint_parent_global.orientation;
        q = joint_local.orientation;
        sk_global.push_back(cpe::skeleton_joint(
            pparent + qparent * p,
            qparent * q));
    }
...
skeleton_geometry inversed(skeleton_geometry const& skeleton)
{
    ...
    for(skeleton_joint const& joint : skeleton)
    {
        ...
        sk_inversed.push_back(skeleton_joint(
            -joint.position,
            conjugated(joint.orientation)
        ));
    }
...
skeleton_geometry multiply(skeleton_geometry const& skeleton_1,skeleton_geometry const& skeleton_2)
{
    ...
    skeleton_joint joint_1, joint_2;
    quaternion q1, q2;
    vec3 p1, p2;
    for(int k=0 ; k<N_joint ; ++k)
    {
        //TO DO: calculer le produit pour chaque joint entre celui du squelette 1 et celui du squelette 2
        joint_1 = skeleton_1[k];
        joint_2 = skeleton_2[k];

        q1 = joint_1.orientation;
        q2 = joint_2.orientation;

        p1 = joint_1.position;
        p2 = joint_2.position;

        sk.push_back(skeleton_joint(q1*p2+p1,
                        q1 *q2));
    }
